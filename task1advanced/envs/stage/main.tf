resource "yandex_vpc_network" "test-network-stage" {
  name                        = "${var.project}-network-${var.environment}"
}

module "test-subnet-stage" {
  source                      = "../../modules/subnet"
  subnet_name                 = "${var.project}-subnet-${var.environment}-a1"
  subnet_network_id           = yandex_vpc_network.test-network-stage.id
  subnet_zone                 = var.zone
  subnet_v4_cidr_blocks       = var.subnet_v4_cidr_blocks

  depends_on = [yandex_vpc_network.test-network-stage]
}

module "test-vm-stage" {
  ...
  instance_subnet_id          = module.test-subnet-stage.id
  instance_nat                = true
  instance_user               = "ubuntu"

  depends_on = [module.test-subnet-stage]     
}
